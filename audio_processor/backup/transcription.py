...
def main():
    ...
    transcription = ['']
    chunks = []  # Store individual chunks

    # Create output files with timestamp
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    chunks_filename = f"chunks_{timestamp}.txt"
    full_filename = f"full_transcription_{timestamp}.txt"
            ...
            if not data_queue.empty():
                ...
                if text:  # Only process if there's actual text
                    ...
                    if phrase_complete:
                        ...
                        chunks.append(text)  # Add to chunks
                    else:
                        transcription[-1] = text
                        if chunks:
                            chunks[-1] = text  # Update last chunk
                        else:
                            chunks.append(text)
